...
function deletePermanentlyDropdown() {
  $('.action-dropdown .dropdown-menu .permanent-delete.dropdown-item').on(
    'click',
    function (event) {
      event.preventDefault()
      if ($(this).parents('.todo-item').hasClass('todo-task-trash')) {
    ...
    $('.edit-tsk')
      ...
      .on('click', function (event) {
        ...
        var $_taskEditedTitleDataAttr = $_outerThis
          ...
          .children()
          .find('.todo-heading')
          .attr('data-todoHeading', $_task)
        var $_taskEditedTextDataAttr = $_outerThis
          .parents('.todo-item')
          .children()
          .find('.todo-text')
          .attr('data-todoText', $_textDelta)
        var $_taskEditedTextDataAttr = $_outerThis
          .parents('.todo-item')
          .children()
          .find('.todo-text')
          .attr('data-todoHtml', $_taskDescriptionInnerHTML)
...
$('.add-tsk').click(function () {
  ...
  $html =
    ...
    '<h5 class="todo-heading" data-todoHeading="' +
    $_task +
    '"> ' +
    $_task +
    '</h5>' +
