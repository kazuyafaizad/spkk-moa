$(function () {
  ...
  $('.addTaskAccordion .collapse').on('shown.bs.collapse', function () {
    ...
    getClassOfAddTaskBtn = $(this)
      .parents('.modal-content')
      .find('[data-btnfn="addTask"]')
      .attr('class')
      .split(' ')[1]
    removeClassOfAddTaskBtn = $(this)
      .parents('.modal-content')
      .find('[data-btnfn="addTask"]')
      .removeClass(getClassOfAddTaskBtn)
    var addClassInAddTaskBtn

    if (getClassOfAccordion === 'task-simple') {
      addClassInAddTaskBtn = $(this)
        .parents('.modal-content')
        .find('[data-btnfn="addTask"]')
        .addClass('task-simple')
    }
    if (getClassOfAccordion === 'task-text-progress') {
      addClassInAddTaskBtn = $(this)
        .parents('.modal-content')
        .find('[data-btnfn="addTask"]')
        .addClass('task-text-progress')
      ...
      .on('click', function (event) {
        ...
        if (getAddBtnClass === 'task-simple') {
          ...
          $html =
            ...
            '<div class="progress-bar bg-success" role="progressbar" data-progressState="' +
            $_taskProgress +
            '" style="width: ' +
            $_taskProgress +
            '%" aria-valuenow="' +
          ...
          $html =
            '<div data-draggable="true" class="card img-task ui-sortable-handle" style="">' +
            '<div class="card-body">' +
            '<div class="task-content">' +
  ...
  function $_editList() {
    $('.list-edit')
      ...
      .on('click', function (event) {
        ...
        var $_listTitle = $_outerThis
          .parents('[data-connect="sorting"]')
          .find('.s-heading')
          .attr('data-listTitle')
        ...
        $('.edit-list')
          ...
          .on('click', function (event) {
            ...
            var $_editedListTitle = $_outerThis
              .parents('[data-connect="sorting"]')
              .find('.s-heading')
              .html($_getListTitle)
            var $_editedListTitleDataAttr = $_outerThis
              .parents('[data-connect="sorting"]')
              .find('.s-heading')
              .attr('data-listTitle', $_getListTitle)
    ...
    $('.card .s-task-edit')
      ...
      .on('click', function (event) {
        ...
        if ($(this).parents('.card').hasClass('img-task')) {
          ...
          var $_taskProgress = $_outerThis
            .parents('.card')
            .find('div.progress-bar')
            .attr('data-progressState')
          ...
          var get_value_progressDataAttr = $('.range-count-number').attr(
        ...
        $('[data-btnfn="editTask"]')
          ...
          .on('click', function (event) {
            ...
            if ($_outerThis.parents('.card').hasClass('img-task')) {
              var $_task = document.getElementById('s-image-task').value
              var $_taskDataAttr = $_outerThis
                .parents('.card')
                .find('h4')
                .attr('data-taskTitle', $_task)
              ...
              var $_taskDataAttr = $_outerThis
                .parents('.card')
                .find('h4')
                .attr('data-taskTitle', $_taskValue)
              var $_taskTitle = $_outerThis.parents('.card').find('h4').html($_taskValue)
              var $_taskTextDataAttr = $_outerThis
                .parents('.card')
                .find('p:not(".progress-count")')
                .attr('data-tasktext', $_taskTextValue)
              var $_taskText = $_outerThis
                .parents('.card')
                .find('p:not(".progress-count")')
                .html($_taskTextValue)

              var $_taskProgressStyle = $_outerThis
                .parents('.card')
                .find('div.progress-bar')
                .attr('style', 'width: ' + $_taskProgressValue + '%')
              var $_taskProgressDataAttr = $_outerThis
                .parents('.card')
                .find('div.progress-bar')
                .attr('data-progressState', $_taskProgressValue)
              var $_taskProgressAriaAttr = $_outerThis
                .parents('.card')
                .find('div.progress-bar')
                .attr('aria-valuenow', $_taskProgressValue)
              var $_taskProgressProgressCount = $_outerThis
                .parents('.card')
                .find('.progress-count')
                .html($_taskProgressValue + '%')
            }
