...
;(jvm.$ = jQuery),
  ...
  (jvm.VMLCanvasElement.prototype.setSize = function (width, height) {
    ...
    if (
      ...
      for (
        paths = this.rootElement.node.getElementsByTagName('shape'), i = 0, l = paths.length;
        l > i;
        i++
      )
  ...
  (jvm.NumericScale.prototype = {
    ...
    getTicks: function () {
      var tick,
        ...
        ticks = []
      for (
        0.15 >= err ? (step *= 10) : 0.35 >= err ? (step *= 5) : 0.75 >= err && (step *= 2),
  ...
  (jvm.Marker = function (config) {
    ...
    ;(this.config = config),
      ...
      this.config.label &&
        ...
        (this.label = config.canvas.addText(
          { text: text, 'data-index': config.index, dy: '0.6ex', x: this.labelX, y: this.labelY },
          config.labelStyle,
          config.labelsGroup
        )),
    ...
    latLngToPoint: function (lat, lng) {
      ...
      return (
        ...
        inset
          ? ((bbox = inset.bbox),
            (point.x =
              ((point.x - bbox[0].x) / (bbox[1].x - bbox[0].x)) * inset.width * this.scale),
            (point.y =
              ((point.y - bbox[0].y) / (bbox[1].y - bbox[0].y)) * inset.height * this.scale),
            {
