...
const edit = ()=>{
     dt.rows({ selected: true }).every(function () {
        router.get(route('admin.recipe.edit',{recipe: this.data()}));
    });
}
...
const destroy = ()=>{
    dt.rows({ selected: true }).every(function () {
        if(confirm("Anda Pasti untuk memadam?")){
            router.delete(route('admin.recipe.destroy',{recipe: this.data()}),{
            onFinish:()=>{
        ...
        <div>
            <div class="w-full max-w-full mb-4 bg-transparent">
                <div class="flex gap-2 my-2">
                    <Link class="btn btn-primary rounded " :href="route('admin.recipe.create')" as="button"
                ...
                <DataTable :data="$page.props.recipe.data" class="display table w-full" :columns="columns"
                    :options="{ select: true }" ref="table"></DataTable>
            </div>
        </div>
    </AppLayout>
</template>
